...
## Book Navigation And Search

Project-book output can add standalone-only navigation chrome around the
document. `book.sidebar` renders a sticky `<aside class="mda-book-sidebar">`
with the generated book outline, and `book.search` renders a local search
control. When both are enabled, search lives in the sidebar; when only search is
enabled, it appears above the document.

The search script is generated by `src/render/html/book-navigation.ts` with the
`data-mda-book-search-script` marker. It builds an in-browser index from
rendered headings and nearby body text, uses `textContent` for result markup,
and does not call external services. VS Code webviews treat it as a first-party
script and add a CSP nonce in the same way as document controls and Mermaid
initialization.
