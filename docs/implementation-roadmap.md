...
| Temporary HTML output | `[Done]` | `src/vscode/preview-markdown-command.ts`, `package.json`, `docs/vscode-extension.md` | VS Code `Preview HTML` renders into a temporary live webview without writing HTML; the in-preview `Save HTML` button and `Save Preview HTML` command render the current Markdown state and persist it on demand. |
