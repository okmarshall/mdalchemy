import { expandTabs, isBlankLine } from "../core/source.js";
import {
  isThematicBreak,
  matchAtxHeading,
  matchFenceStart,
...
export function isParagraphInterrupt(text: string): boolean {
  ...
  if (listMarker) return listMarker.content.trim().length > 0 && (!listMarker.ordered || listMarker.start === 1);
  if (isThematicBreak(text)) return true;
  const htmlStart = matchHtmlBlockStart(text);
  if (htmlStart) return htmlStart.kind !== "complete";
  return false;
}
